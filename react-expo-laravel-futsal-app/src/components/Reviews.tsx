...
import { Star, Trash2, MessageCircleHeart, Loader2 } from "lucide-react";
...
export function ReviewsSection({
  ...
  async function remove(id: number) {
    ...
    await fetch(`/api/reviews?id=${id}&userId=${user.id}`, { method: "DELETE" });
    await load();
    onChanged?.();
  }
  ...
  return (
    <div className={compact ? "" : "rounded-3xl border border-[#F0E3CC] bg-white p-5 shadow-sm dark:border-white/10 dark:bg-stone-900"}>
      ...
      {canReview && !showForm && (
        ...
        >
          Played at {venueName}? Share some love ⭐
        </button>
      )}
              ...
              <div className="flex items-center gap-2.5">
                ...
                {r.userId === user?.id && (
                  <button
                    ...
                    className="grid h-7 w-7 place-items-center rounded-lg text-stone-300 hover:bg-red-50 hover:text-red-500 dark:text-stone-600"
                  >
