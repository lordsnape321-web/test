...
export function SwipeNotificationRow({
  ...
  const surface = n.isRead
    ...
    : owner
      ? "border-emerald-300 bg-emerald-50 ring-1 ring-emerald-300 dark:border-emerald-400/50 dark:bg-emerald-500/10 dark:ring-emerald-400/30"
  ...
  return (
    ...
    >
      ...
      >
        <span className={`mt-0.5 shrink-0 rounded-lg px-2.5 py-1.5 text-[10px] font-black uppercase ${typeClass}`}>
          {n.type.replace(/_/g, " ")}
        </span>
        <button
          onClick={() => {
        ...
        >
          ...
          <p className={`mt-1.5 text-[11px] font-bold ${owner ? "text-slate-400 dark:text-slate-500" : "text-stone-400 dark:text-slate-500"}`}>
            {timeAgo(n.createdAt)}
          </p>
        </button>
        {n.link && (
          <button
            type="button"
            onClick={() => onOpen(n)}
            className={`mt-1 shrink-0 rounded-full border px-2.5 py-1.5 text-[11px] font-black transition ${owner ? "border-orange-200 bg-orange-50 text-orange-700 hover:bg-orange-100 dark:border-orange-400/30 dark:bg-orange-500/10 dark:text-orange-300" : "border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 dark:border-emerald-400/30 dark:bg-emerald-500/10 dark:text-emerald-300"}`}
          >
            {owner ? "Open" : "Have a look"} <ChevronRight className="inline h-3 w-3" />
          </button>
        )}
      </div>
    </div>
