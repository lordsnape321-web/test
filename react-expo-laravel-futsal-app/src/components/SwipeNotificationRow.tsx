  ...
  return (
    ...
    >
      <div className="absolute inset-0 flex items-center justify-between bg-slate-100 px-5 text-xs font-black dark:bg-slate-800">
        <span className={`flex items-center gap-1.5 text-emerald-700 transition-opacity dark:text-emerald-300 ${offset > 8 ? "opacity-100" : "opacity-35"}`}>
          <ArrowRight className="h-4 w-4" />
          <Check className="h-4 w-4" /> Mark read
        </span>
        <span className={`flex items-center gap-1.5 text-red-600 transition-opacity dark:text-red-400 ${offset < -8 ? "opacity-100" : "opacity-35"}`}>
      ...
      >
        <span className={`mt-0.5 shrink-0 rounded-lg px-2.5 py-1.5 text-[10px] font-black uppercase ${typeClass}`}>
          {!n.isRead && !owner ? "● " : ""}{n.type.replace(/_/g, " ")}
        </span>
        <button
          onClick={() => {
