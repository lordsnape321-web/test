...
export function VenueCard({ v }: { v: VenueWithCourts }) {
  return (
    <Link
      href={`/venues/${v.id}`}
      className="card-glow group overflow-hidden rounded-3xl border border-[#F0E3CC] bg-white shadow-[0_10px_30px_rgba(180,120,60,0.08)] transition hover:-translate-y-1 hover:border-emerald-300 dark:border-white/10 dark:bg-slate-900 dark:hover:border-emerald-500/50"
    >
      <div className="relative h-48 overflow-hidden">
        ...
        <span className="absolute right-3 top-3 flex items-center gap-1 rounded-full bg-white/95 px-2.5 py-1 text-xs font-bold text-stone-800 shadow backdrop-blur dark:bg-slate-900/95 dark:text-slate-100">
          <Star className="h-3 w-3 fill-amber-400 text-amber-400" />
          {v.rating.toFixed(1)}
          <span className="font-medium text-stone-400 dark:text-slate-500">({v.totalReviews})</span>
        </span>
        <span className="absolute bottom-3 left-3 rounded-full bg-white/95 px-2.5 py-1 text-[11px] font-bold text-emerald-700 shadow backdrop-blur dark:bg-slate-900/95 dark:text-emerald-300">
          From {formatNPR(v.minPrice)}/hr
        </span>
      </div>
      <div className="p-4">
        <h3 className="truncate text-[15px] font-extrabold text-stone-900 group-hover:text-emerald-700 dark:text-slate-100 dark:group-hover:text-emerald-400">
          {v.name}
        </h3>
        <p className="mt-1 flex items-center gap-1 truncate text-xs text-stone-500 dark:text-slate-400">
          <MapPin className="h-3.5 w-3.5 shrink-0" />
          {v.address} • {v.city}
        </p>
        <div className="mt-3 flex items-center justify-between border-t border-stone-100 pt-3 dark:border-white/5">
          <span className="flex items-center gap-1 text-xs font-semibold text-stone-500 dark:text-slate-400">
...
export function MatchCard({ m }: { m: MatchItem }) {
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-[#F0E3CC] bg-white p-5 shadow-[0_10px_30px_rgba(180,120,60,0.08)] transition hover:border-emerald-300 dark:border-white/10 dark:bg-slate-900 dark:hover:border-emerald-500/50">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="text-[15px] font-extrabold leading-snug text-stone-900 dark:text-slate-100">{m.title}</h3>
      ...
      <p className="mt-2.5 flex items-center gap-1.5 truncate text-xs text-stone-500 dark:text-slate-400">
        <MapPin className="h-3.5 w-3.5 shrink-0" />
        ...
        <div className="flex -space-x-2">
          {(m.players ?? []).slice(0, 5).map((p) => (
            <span key={p.id} title={p.name}>
              <Avatar user={{ name: p.name, avatarColor: p.avatarColor, avatarUrl: p.avatarUrl }} className="h-7 w-7 text-[10px]" ring="border-2 border-white shadow dark:border-slate-900" />
            </span>
          ))}
          {m.joinedCount > 5 && (
            <span className="grid h-7 w-7 place-items-center rounded-full border-2 border-white bg-stone-200 text-[10px] font-black text-stone-600 dark:border-slate-900 dark:bg-white/10 dark:text-slate-300">
              +{m.joinedCount - 5}
            </span>
          )}
        </div>
