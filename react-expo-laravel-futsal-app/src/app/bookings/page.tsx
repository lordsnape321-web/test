...
export default function BookingsPage() {
  ...
  const [moneyRequestPayer, setMoneyRequestPayer] = useState<Record<number, string>>({});
  ...
  async function requestTeamMoney(b: Booking) {
    ...
    const payerId = Number(moneyRequestPayer[b.id] ?? 0);
    const amount = Number(moneyRequestAmount[b.id] ?? 0);
    if (!payerId || !Number.isInteger(amount) || amount < 10) {
      setPayError("Choose a teammate and enter at least Rs. 10.");
      return;
    }
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || "Could not send the payment request");
      setMoneyRequestFor(null);
      setMoneyRequestPayer((current) => ({ ...current, [b.id]: "" }));
      setMoneyRequestAmount((current) => ({ ...current, [b.id]: "" }));
      setMoneyRequestNote((current) => ({ ...current, [b.id]: "" }));
      await load(user.id);
    } catch (e) {
      setPayError(e instanceof Error ? e.message : "Could not send the payment request");
    } finally {
                      ...
                      <div className="mt-2.5 rounded-2xl border border-orange-200 bg-orange-50/70 p-3.5 dark:border-orange-500/25 dark:bg-orange-500/10">
                        ...
                        {moneyRequestFor === b.id && (
                          <div className="mt-3 grid gap-2 sm:grid-cols-2">
                            <select
                              value={moneyRequestPayer[b.id] ?? ""}
                              onChange={(event) => setMoneyRequestPayer((current) => ({ ...current, [b.id]: event.target.value }))}
                              className="rounded-xl border border-orange-200 bg-white px-3 py-2 text-xs font-bold text-stone-700 dark:border-white/10 dark:bg-slate-950 dark:text-slate-100"
                            >
                              <option value="">Choose a player</option>
                              {b.teamPlayers.filter((player) => player.id !== user?.id).map((player) => (
                                <option key={player.id} value={player.id}>{player.name}{player.role ? ` · ${player.role}` : ""}</option>
                              ))}
                            </select>
                            <input
                              inputMode="decimal"
                              ...
                              className="rounded-xl border border-orange-200 bg-white px-3 py-2 text-xs font-bold text-stone-700 placeholder:text-stone-400 dark:border-white/10 dark:bg-slate-950 dark:text-slate-100"
                            />
                            <input
