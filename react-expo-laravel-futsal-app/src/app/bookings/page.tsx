...
export default function BookingsPage() {
  ...
  function needsOnlinePay(b: Booking) {
    ...
    if (b.advancePaymentRequired && b.advancePaymentStatus !== "paid") return true;
                ...
                <div className="flex flex-col sm:flex-row">
                  ...
                  <div className="flex-1 p-4 sm:p-5">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      ...
                      <span className="text-right">
                        ...
                        <span className="flex items-center gap-1 text-[11px] font-bold text-stone-400 dark:text-slate-500">
                          <Wallet className="h-3 w-3" /> {b.paymentStatus === "paid" ? `Paid via ${b.paymentMethod}` : `Payment method: ${b.paymentMethod}`} • Payment status: {paymentStatusLabel(b.paymentStatus)}
                        </span>
                      </span>
                    </div>
                    ...
                    {b.advancePaymentRequired && (
                      <div className="mt-2.5 rounded-xl border border-sky-200 bg-sky-50 px-3.5 py-2 text-xs font-bold leading-relaxed text-sky-800 dark:border-sky-500/25 dark:bg-sky-500/10 dark:text-sky-200">
                        💳 Venue advance: {formatNPR(b.advancePaymentAmount)} • {b.advancePaymentStatus === "paid" ? "paid and verified ✓" : "payment requested — choose your gateway below"}
                      </div>
                    )}
                    {b.teamName && b.teamPayments?.length > 0 && myTeamShare && (
                      <div className="mt-2.5 rounded-2xl border border-sky-200 bg-sky-50/80 p-3.5 dark:border-sky-500/25 dark:bg-sky-500/10">
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          <span className="rounded-full bg-white/80 px-3 py-1.5 text-[11px] font-black text-sky-800 dark:bg-white/10 dark:text-sky-100">
                            {myTeamShare.paymentStatus === "paid" ? `✓ Paid via ${myTeamShare.paymentMethod}` : myTeamShare.paymentMethod ? `Payment method: ${myTeamShare.paymentMethod}` : "Payment method not chosen"} • Payment status: {paymentStatusLabel(myTeamShare.paymentStatus)}
                          </span>
                          <button
                            type="button"
                            onClick={() => {
                        ...
                        {teamMethodFor === b.id && (
                          <div className="mt-3 rounded-xl border border-sky-200 bg-white/80 p-3 dark:border-sky-400/25 dark:bg-slate-950/40">
                            ...
                            <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-3">
                              {["eSewa", "Khalti", "Cash at Venue"].map((method) => (
                                <button
                                  key={method}
                                  type="button"
