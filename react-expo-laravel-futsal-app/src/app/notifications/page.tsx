    ...
    return (
      <main className="turf-pattern grid min-h-screen place-items-center px-4 py-12">
        <div className="w-full max-w-md rounded-[2rem] border border-[#F0E3CC] bg-white p-8 text-center shadow-lg dark:border-white/10 dark:bg-stone-900">
          ...
          <h1 className="mt-4 text-2xl font-black text-stone-900 dark:text-stone-100">Your letters await 💌</h1>
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-black text-stone-900 dark:text-stone-100">
          ...
          {unread > 0 && (
            <button
              onClick={markAll}
              className="flex shrink-0 items-center gap-1.5 rounded-full border border-stone-200 bg-white px-4 py-2 text-xs font-black text-stone-700 shadow-sm transition hover:bg-stone-50 dark:border-white/10 dark:bg-stone-900 dark:text-stone-200 dark:hover:bg-white/5"
            >
        ...
        {loading ? (
          <div className="mt-6 space-y-2.5">
            {[0, 1, 2].map((i) => (
              <div key={i} className="h-20 animate-pulse rounded-2xl bg-white dark:bg-stone-900" />
            ))}
          </div>
        ) : items.length === 0 ? (
          <div className="mt-6 rounded-3xl border border-dashed border-stone-300 bg-white p-12 text-center dark:border-white/20 dark:bg-stone-900">
            ...
            <h3 className="mt-3 text-lg font-extrabold text-stone-900 dark:text-stone-100">All quiet, all good 🎉</h3>
            ...
            {items.map((n) => (
              <div
                ...
                className={`group flex items-start gap-3 rounded-2xl border p-4 shadow-sm transition ${
                  n.isRead
                    ? "border-stone-200 bg-white dark:border-white/10 dark:bg-stone-900"
              ...
              >
                ...
                <button onClick={() => markOne(n)} className="min-w-0 flex-1 text-left">
                  <p className="text-sm font-extrabold leading-snug text-stone-900 dark:text-stone-100">{n.title}</p>
                  {n.message && (
                    <p className="mt-1 text-[13px] leading-relaxed text-stone-500 dark:text-stone-400">
                      {n.message}
                    </p>
                  )}
                  <p className="mt-1.5 flex items-center gap-1 text-[11px] font-bold text-stone-400 dark:text-stone-500">
                    {timeAgo(n.createdAt)}
                    {n.link && (
                ...
                <button
                  onClick={() => remove(n.id)}
                  title="Delete"
                  className="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-stone-300 transition hover:bg-red-50 hover:text-red-500 dark:text-stone-600 dark:hover:bg-red-500/10 dark:hover:text-red-400"
                >
