...
export async function POST(
  ...
  try {
    ...
    const payerId = Number(body.payerId ?? 0);
    const amount = Number(body.amount ?? 0);
    const purpose = String(body.purpose ?? "booking") as PaymentPurpose;
    const note = String(body.note ?? "").trim().slice(0, 240);
    if (!Number.isInteger(bookingId) || bookingId <= 0 || !Number.isInteger(requesterId) || requesterId <= 0 || !Number.isInteger(payerId) || payerId <= 0) {
      return Response.json({ error: "Pick a valid booking and player 🔒" }, { status: 400 });
    }
    ...
    if (payerId === requesterId) return Response.json({ error: "Choose another team player for the request" }, { status: 400 });
    const payerMember = await db.select().from(teamMembers).where(and(eq(teamMembers.teamId, booking.teamId), eq(teamMembers.userId, payerId)));
    if (payerMember.length === 0) return Response.json({ error: "That player is not on this booking's team 🔒" }, { status: 403 });
    ...
    if (amount > remaining) {
      return Response.json({ error: `Only ${formatNPR(remaining)} remains available for this payment request` }, { status: 400 });
    }
    ...
    return Response.json({ paymentRequest: rowView(request, people) }, { status: 201 });
  } catch (e) {
