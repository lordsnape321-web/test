...
const ROW =
  "flex w-full items-center gap-3 rounded-2xl border border-[#F0E3CC] bg-white px-4 py-3.5 text-left shadow-sm transition hover:border-emerald-300 dark:border-white/10 dark:bg-stone-900 dark:hover:border-emerald-500/50";
    ...
    return (
      <main className="turf-pattern grid min-h-screen place-items-center px-4 py-12">
        <div className="w-full max-w-md rounded-[2rem] border border-[#F0E3CC] bg-white p-8 text-center shadow-lg dark:border-white/10 dark:bg-stone-900">
          ...
          <h1 className="mt-4 text-2xl font-black text-stone-900 dark:text-stone-100">
    ...
    return (
      <main className="turf-pattern min-h-screen">
        <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
          <div className="h-24 animate-pulse rounded-3xl bg-white dark:bg-stone-900" />
        </div>
      </main>
    );
  }
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="min-w-0">
            <h1 className="truncate text-2xl font-black text-stone-900 sm:text-3xl dark:text-stone-100">
              Settings
            </h1>
                  ...
                  <button
                    ...
                    className={`flex shrink-0 items-center gap-2.5 rounded-2xl px-4 py-3 text-sm font-bold transition lg:w-full ${
                      active
                        ...
                        : "border border-[#F0E3CC] bg-white text-stone-700 hover:bg-orange-50 dark:border-white/10 dark:bg-stone-900 dark:text-stone-200 dark:hover:bg-white/5"
                    }`}
                  >
          ...
          <div className="mt-4 min-w-0 lg:mt-0">
              ...
              <section className="space-y-3">
                ...
                <div className="rounded-3xl border border-[#F0E3CC] bg-white p-5 shadow-sm dark:border-white/10 dark:bg-stone-900">
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-base font-black text-stone-900 dark:text-stone-100">
                        {user.name}
                      </p>
                ...
                <label className={ROW}>
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block text-sm font-black text-stone-900 dark:text-stone-100">
                      Home city
                    </span>
                  ...
                  <select
                    ...
                    className="w-36 shrink-0 rounded-xl border border-stone-200 bg-[#FFF6E9] px-2.5 py-2 text-xs font-bold text-stone-900 focus:border-emerald-500 focus:outline-none disabled:opacity-50 dark:border-white/10 dark:bg-white/5 dark:text-stone-100"
                  >
                    {CITY_OPTIONS.map((c) => (
            ...
            {section === "alerts" && (
              <section className="space-y-3">
                ...
                />
                <div className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-[#F0E3CC] bg-white px-5 py-4 shadow-sm dark:border-white/10 dark:bg-stone-900">
                  <p className="text-sm font-black text-stone-900 dark:text-stone-100">
                    {unread > 0 ? `${unread} unread` : "All caught up 🎉"}
                    <span className="ml-2 text-xs font-semibold text-stone-400 dark:text-stone-500">
                      {notes.length} total
                    </span>
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {unread > 0 && (
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-xl border border-stone-200 px-3.5 py-2 text-xs font-black text-stone-700 transition hover:bg-stone-50 dark:border-white/10 dark:text-stone-200 dark:hover:bg-white/5"
                      >
                ...
                {notesLoading ? (
                  <div className="space-y-2.5">
                    {[0, 1, 2].map((i) => (
                      <div key={i} className="h-16 animate-pulse rounded-2xl bg-white dark:bg-stone-900" />
                    ))}
                  </div>
                ) : notes.length === 0 ? (
                  <p className="rounded-3xl border border-dashed border-stone-300 bg-white p-8 text-center text-sm font-semibold text-stone-400 dark:border-white/10 dark:bg-stone-900 dark:text-stone-500">
                    ...
                    {notes.slice(0, 6).map((n) => (
                      <Link
                        ...
                        className={`flex items-start gap-3 rounded-2xl border p-4 shadow-sm transition ${
                          n.isRead
                            ? "border-[#F0E3CC] bg-white dark:border-white/10 dark:bg-stone-900"
                      ...
                      >
                        <span className="min-w-0 flex-1">
                          ...
                          {n.message && (
                            <span className="mt-0.5 line-clamp-2 block text-xs leading-relaxed text-stone-500 dark:text-stone-400">
                              {n.message}
                            </span>
                          )}
                          <span className="mt-1 block text-[11px] font-bold text-stone-400 dark:text-stone-500">
                            {timeAgo(n.createdAt)}
                          </span>
                        </span>
                        <ChevronRight className="mt-1 h-4 w-4 shrink-0 text-stone-300 dark:text-stone-600" />
                      </Link>
                    ))}
                  </div>
                )}
                      ...
                      <button
                        ...
                        className={`rounded-3xl border p-5 text-left shadow-sm transition ${
                          active
                            ...
                            : "border-[#F0E3CC] bg-white hover:border-emerald-300 dark:border-white/10 dark:bg-stone-900"
                        }`}
                      >
                        <span
                          className={`grid h-11 w-11 place-items-center rounded-2xl ${
                            active
                              ...
                              : "bg-[#FFF6E9] text-stone-500 dark:bg-white/5 dark:text-stone-400"
                          }`}
                        >
                          <t.icon className="h-5 w-5" />
                        </span>
                        <span className="mt-3 block text-sm font-black text-stone-900 dark:text-stone-100">
                          {t.label} {active && "✓"}
                        </span>
                        <span className="block text-xs text-stone-500 dark:text-stone-400">{t.text}</span>
                      </button>
                    );
                  })}
                </div>
            ...
            {section === "help" && (
              <section className="space-y-3">
                ...
                />
                <div className="space-y-3 rounded-3xl border border-[#F0E3CC] bg-white p-5 text-sm leading-relaxed text-stone-600 shadow-sm dark:border-white/10 dark:bg-stone-900 dark:text-stone-300">
                  <p>
                    <strong className="text-stone-900 dark:text-stone-100">Booking.</strong> A
                    real person at the venue confirms every request — you will get an alert when
                    they do.
                  </p>
                  <p>
                    <strong className="text-stone-900 dark:text-stone-100">Paying.</strong> eSewa
                    and Khalti run in test mode here, and cash at the counter is always fine.
                  </p>
                  <p>
                    <strong className="text-stone-900 dark:text-stone-100">Leagues.</strong> A
                  ...
                  <p>
                    <strong className="text-stone-900 dark:text-stone-100">Where things live.</strong>{" "}
...
function PanelHead({
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0">
        <h2 className="text-lg font-black text-stone-900 dark:text-stone-100">{title}</h2>
        <p className="text-xs leading-relaxed text-stone-500 dark:text-stone-400">{text}</p>
      </div>
    </div>
  );
}
...
function ActivityTile({
  ...
  return (
    <Link
      href={href}
      className="rounded-3xl border border-[#F0E3CC] bg-white p-5 shadow-sm transition hover:border-emerald-300 dark:border-white/10 dark:bg-stone-900 dark:hover:border-emerald-500/50"
    >
      ...
      <span className="mt-3 flex items-center gap-1 text-sm font-black text-stone-900 dark:text-stone-100">
        {title} <ChevronRight className="h-4 w-4 text-stone-300 dark:text-stone-600" />
      </span>
...
function AccountRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 border-b border-stone-100 pb-2.5 last:border-0 last:pb-0 dark:border-white/5">
      <dt className="text-xs font-black uppercase tracking-wider text-stone-400 dark:text-stone-500">
        {label}
      </dt>
      <dd className="min-w-0 flex-1 truncate text-right text-sm font-bold text-stone-800 dark:text-stone-100">
        {value}
      </dd>
    </div>
  );
