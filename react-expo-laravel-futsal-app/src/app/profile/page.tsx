...
const SELECT_CLS =
  "w-full rounded-xl border border-stone-200 bg-[#FFF6E9] px-3.5 py-2.5 text-sm font-semibold text-stone-900 focus:border-emerald-500 focus:outline-none dark:border-white/10 dark:bg-white/5 dark:text-stone-100 [&>option]:bg-white [&>option]:text-stone-900 dark:[&>option]:bg-stone-900 dark:[&>option]:text-stone-100";
    ...
    return (
      <main className="turf-pattern grid min-h-screen place-items-center px-4 py-12">
        <div className="w-full max-w-md rounded-[2rem] border border-[#F0E3CC] bg-white p-8 text-center shadow-lg dark:border-white/10 dark:bg-stone-900">
          ...
          <h1 className="mt-4 text-2xl font-black text-stone-900 dark:text-stone-100">Your profile awaits 🌟</h1>
          <p className="mt-2 text-sm text-stone-500 dark:text-stone-400">Log in to style your player card.</p>
          <div className="mt-6 grid grid-cols-2 gap-2">
            ...
            <Link href="/signup" className="rounded-2xl border border-stone-200 py-3 text-sm font-black text-stone-700 dark:border-white/10 dark:text-stone-200">
              Join free
            </Link>
          </div>
        </div>
  ...
  return (
    <main className="turf-pattern min-h-screen">
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        ...
        <h1 className="mt-1 text-3xl font-black text-stone-900 dark:text-stone-100">My profile</h1>
        ...
        {(vouchers.length > 0 || progress.length > 0) && (
          <div className="mt-4 rounded-3xl border border-[#F0E3CC] bg-white p-5 shadow-sm dark:border-white/10 dark:bg-stone-900">
        ...
        <div className="mt-4 rounded-3xl border border-[#F0E3CC] bg-white p-5 shadow-sm dark:border-white/10 dark:bg-stone-900">
          ...
          <div className="mt-4 min-w-0 border-t border-stone-100 pt-3 dark:border-white/5">
            <p className="truncate text-xl font-black text-stone-900 dark:text-stone-100">{name || "…"}</p>
            <p className="truncate text-sm text-stone-500 dark:text-stone-400">{user?.email}</p>
            <p className="mt-0.5 text-xs font-bold text-stone-400 dark:text-stone-500">
        ...
        <div className="mt-4 rounded-3xl border border-[#F0E3CC] bg-white p-5 shadow-sm dark:border-white/10 dark:bg-stone-900">
          ...
          <div className="mt-3 space-y-3">
            <label className="block">
              <span className="mb-1.5 block text-xs font-black uppercase tracking-wider text-stone-400 dark:text-stone-500">Display name</span>
              <input
                value={name}
                onChange={(e) => {
            ...
            <label className="block">
              ...
              <span className={`flex items-center gap-2 rounded-xl border bg-[#FFF6E9] px-4 dark:bg-white/5 ${
                ...
                <input
                  ...
                  maxLength={16}
                  inputMode="tel"
                  className="w-full bg-transparent py-2.5 text-sm font-semibold text-stone-900 focus:outline-none dark:text-stone-100"
                />
              </span>
              {fieldErrors.phone && <span className="mt-1 block text-[11px] font-bold text-red-500">{fieldErrors.phone}</span>}
            </label>
            <label className="block">
              <span className="mb-1.5 flex items-center gap-1.5 text-xs font-black uppercase tracking-wider text-stone-400 dark:text-stone-500">
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="block">
                <span className="mb-1.5 block text-xs font-black uppercase tracking-wider text-stone-400 dark:text-stone-500">Level</span>
              ...
              <label className="block">
                <span className="mb-1.5 block text-xs font-black uppercase tracking-wider text-stone-400 dark:text-stone-500">Position</span>
            ...
            <div>
              <span className="mb-1.5 block text-xs font-black uppercase tracking-wider text-stone-400 dark:text-stone-500">Avatar colour (backup if no photo)</span>
                ...
                {COLORS.map((c) => (
                  <button
                    ...
                    className={`grid h-9 w-9 place-items-center rounded-full transition ${
                      color === c ? "ring-2 ring-emerald-500 ring-offset-2 ring-offset-white dark:ring-offset-stone-900" : ""
                    }`}
                    style={{ background: c }}
                  >
        ...
        <div className="mt-4 rounded-3xl border border-[#F0E3CC] bg-white p-5 shadow-sm dark:border-white/10 dark:bg-stone-900">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <label className="block">
              <span className="mb-1.5 block text-xs font-black uppercase tracking-wider text-stone-400 dark:text-stone-500">Current password</span>
              <input
                type={showPw ? "text" : "password"}
                value={currentPw}
                onChange={(e) => setCurrentPw(e.target.value)}
                placeholder="••••••••"
                className="w-full rounded-xl border border-stone-200 bg-[#FFF6E9] px-3.5 py-2.5 text-sm font-semibold text-stone-900 focus:border-orange-400 focus:outline-none dark:border-white/10 dark:bg-white/5 dark:text-stone-100"
              />
            </label>
            <label className="block">
              <span className="mb-1.5 block text-xs font-black uppercase tracking-wider text-stone-400 dark:text-stone-500">New password</span>
