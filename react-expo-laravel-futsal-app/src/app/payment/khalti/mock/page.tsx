...
function Inner() {
  ...
  async function pay() {
    ...
    try {
      ...
      router.push(`/payment/khalti/callback?pidx=${encodeURIComponent(pidx)}&bookingId=${bookingId}${teamPaymentId ? `&teamPaymentId=${teamPaymentId}` : ""}&status=Completed&mock=1`);
    } catch (e) {
