    ...
    <main className="turf-pattern min-h-screen">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 pb-12 pt-10 sm:px-6 lg:grid-cols-2 lg:pt-16">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-orange-200 bg-white px-3.5 py-1.5 text-xs font-bold text-stone-700 shadow-sm dark:border-orange-500/30 dark:bg-slate-900 dark:text-slate-200">
              <Sparkles className="h-3.5 w-3.5 text-orange-500" />
              {user ? `Welcome back, ${user.name.split(" ")[0]}! Your game misses you ⚽` : "Nepal's friendliest futsal family ⚽"}
            </div>
            <h1 className="mt-5 text-4xl font-black leading-[1.08] tracking-tight text-stone-900 sm:text-5xl lg:text-[3.6rem] dark:text-slate-50">
              Grab your friends.{" "}
            ...
            <p className="mt-4 max-w-lg text-[15px] leading-relaxed text-stone-600 dark:text-slate-400">
            ...
            <form
              ...
              className="mt-6 rounded-3xl border border-[#F0E3CC] bg-white p-2 shadow-[0_16px_40px_rgba(180,120,60,0.12)] dark:border-white/10 dark:bg-slate-900"
            >
            ...
            {user && homeCity !== "All Cities" && (
              <p className="mt-2 text-[11px] font-bold text-stone-400 dark:text-slate-500">
                🏠 Searching in your home city <span className="text-stone-600 dark:text-slate-300">{homeCity}</span> —{" "}
            ...
            <div className="mt-5 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs font-semibold text-stone-500 dark:text-slate-400">
            ...
            <div className="mt-7 grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
              {[
                ...
                <div
                  key={s.l}
                  className="rounded-2xl border border-[#F0E3CC] bg-white px-2 py-3 text-center shadow-sm dark:border-white/10 dark:bg-slate-900"
                >
                  <p className="text-xl font-black text-stone-900 sm:text-2xl dark:text-slate-100">{s.n}</p>
          ...
          <div className="relative hidden lg:block">
            <div className="animate-float-slow relative overflow-hidden rounded-[2rem] border-4 border-white shadow-[0_30px_70px_rgba(180,120,60,0.25)] dark:border-slate-900">
              <img
              ...
              <div className="absolute left-5 right-5 top-5 flex items-center justify-between rounded-2xl bg-white/95 p-3 shadow-lg backdrop-blur-xl dark:bg-slate-900/95">
                <div className="flex items-center gap-3">
                  <span className="grid h-10 w-10 place-items-center rounded-xl bg-emerald-600">
                    <CalendarCheck className="h-5 w-5 text-white" />
                  </span>
                  <div>
                    <p className="text-xs font-black text-stone-900 dark:text-slate-100">Saturday with the gang 🎉</p>
                    <p className="text-[11px] text-stone-500 dark:text-slate-400">Arena A • Today • 7:00 PM • 5v5</p>
                  </div>
                </div>
              ...
              <div className="absolute bottom-5 left-5 right-5 grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-white/95 p-3.5 shadow-lg backdrop-blur-xl dark:bg-slate-900/95">
                  <p className="flex items-center gap-1.5 text-[11px] font-bold text-stone-400 dark:text-slate-500">
                    <Clock className="h-3.5 w-3.5" /> NEXT FREE SLOT
                  </p>
                  <p className="mt-1 text-lg font-black text-stone-900 dark:text-slate-100">Today, 8 PM</p>
            ...
            {/*
              ...
              */}
            <div className="animate-float-slow absolute left-5 top-[calc(50%-2.1rem)] z-10 flex max-w-[calc(100%-2.5rem)] items-center gap-2.5 rounded-2xl border border-white/60 bg-white/95 p-3 pr-5 shadow-xl backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/95">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-amber-100 dark:bg-amber-500/15">
                <Star className="h-5 w-5 fill-amber-400 text-amber-400" />
              </span>
              <div className="min-w-0">
                <p className="text-sm font-black text-stone-900 dark:text-slate-100">4.8 / 5.0</p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
        <div className="flex flex-col items-start gap-3 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between sm:gap-4">
          <div className="min-w-0">
            ...
            <h2 className="mt-1 text-2xl font-black text-stone-900 sm:text-3xl dark:text-slate-100">
              Courts our players love
            </h2>
            <p className="mt-1 text-sm text-stone-500 dark:text-slate-400">Hand-picked, honestly priced, always welcoming.</p>
          </div>
          <Link
            href="/venues"
            className="flex shrink-0 items-center gap-1 rounded-full border border-stone-200 bg-white px-4 py-2 text-xs font-black text-stone-700 shadow-sm transition hover:bg-stone-50 dark:border-white/10 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-white/5"
          >
      ...
      {/* HOW IT WORKS */}
      <section className="border-y border-[#F0E3CC] bg-white/60 dark:border-white/10 dark:bg-slate-900/40">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
          ...
          <h2 className="mt-1 text-center text-2xl font-black text-stone-900 sm:text-3xl dark:text-slate-100">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {[
              ...
              <div
                key={s.step}
                className="relative overflow-hidden rounded-3xl border border-[#F0E3CC] bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-900"
              >
                ...
                <h3 className="mt-4 text-lg font-extrabold text-stone-900 dark:text-slate-100">{s.title}</h3>
                <p className="mt-1.5 text-sm leading-relaxed text-stone-500 dark:text-slate-400">{s.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
        <div className="flex flex-col items-start gap-3 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between sm:gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-stone-500 dark:text-slate-400">No team needed — just bring yourself, we&apos;ll handle the rest.</p>
          </div>
          <Link
            href="/matches"
      ...
      {/* FOOTER */}
      <footer className="border-t border-[#F0E3CC] bg-white/70 dark:border-white/10 dark:bg-slate-950/70">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-4 py-8 sm:flex-row sm:px-6">
          <p className="text-sm font-bold text-stone-900 dark:text-slate-100">
            ...
            <span className="font-medium text-stone-400 dark:text-slate-500">— made with 💚 for players, by players</span>
          </p>
          <div className="flex gap-5 text-xs font-bold text-stone-500 dark:text-slate-400">
