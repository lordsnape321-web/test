...
export function EsewaMockScreen() {
  ...
  return <GatewayMock kind="esewa" params={params} onDone={(id) => router.replace(leagueId ? `/leagues/${leagueId}?paid=1` : `/payment/esewa/success?mock=1&bookingId=${id}`)} onCancel={(id) => router.replace(leagueId ? `/leagues/${leagueId}` : `/payment/esewa/failure?bookingId=${id}`)} />;
}
...
export function KhaltiMockScreen() {
  ...
  const router = useRouter();
  const leagueId = one(params.leagueId);
  return <GatewayMock kind="khalti" params={params} onDone={(id) => router.replace(leagueId ? `/leagues/${leagueId}?paid=1` : `/payment/khalti/callback?mock=1&pidx=${encodeURIComponent(one(params.pidx))}&bookingId=${id}&status=Completed`)} onCancel={(id) => router.replace(leagueId ? `/leagues/${leagueId}` : `/payment/khalti/callback?bookingId=${id}&status=User%20canceled`)} />;
}
  ...
  async function pay() {
    ...
    try {
      if (isLeague) {
        ...
        await verifyEsewa(Number(bookingId), true, teamPaymentId ? Number(teamPaymentId) : undefined);
      } else {
        await verifyKhalti(Number(bookingId), one(params.pidx) || "mock-pidx", true, teamPaymentId ? Number(teamPaymentId) : undefined);
      }
      if (isLeague) {
...
export function KhaltiCallbackScreen() {
  ...
  useEffect(() => {
    ...
    verifyKhalti(Number(bookingId), pidx, false, teamPaymentId ? Number(teamPaymentId) : undefined)
