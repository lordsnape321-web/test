...
function useOwnerBadges(userId: number | undefined) {
  ...
  useEffect(() => {
    void load();
    // The request screen can accept the final item without changing routes.
    // Listen for its authoritative refresh so every Owner Studio surface drops
    // the badge immediately; polling remains the cross-device fallback.
    const subscription = DeviceEventEmitter.addListener("owner-bookings-changed", () => {
      void load();
    });
...
export function OwnerTabBar({
  ...
  return (
    ...
    >
      {OWNER_TABS.map((t, i) => {
        // The hidden `leagues/[id]` route is appended after the six visible
        // tabs. Looking only at state.index therefore leaves every item looking
        // unselected in the control room. The pathname is the stable source of
        // truth for the scoped Leagues tab.
        const focused =
          t.name === "leagues"
            ? pathname === "/admin/leagues" || pathname.startsWith("/admin/leagues/")
            : state.index === i;
        const route = state.routes.find((candidate) => candidate.name === t.name) ?? state.routes[i];
        ...
        return (
          <Pressable
            key={t.name}
            onPress={() => {
              // A detail route is still inside the Leagues tab. Pressing the
              // already-selected item should return to the list, not no-op on
              // the hidden route or try to render the player shell.
              if (focused && !(t.name === "leagues" && pathname !== "/admin/leagues")) return;
              // expo-router's emit return shape varies; navigating on plain
