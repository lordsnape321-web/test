...
/** PATCH /api/bookings/:id → save the player's gateway choice for an advance. */
export function chooseBookingPayment(
  bookingId: number,
  userId: number,
  ...
  return apiJson(`/api/bookings/${bookingId}`, {
    method: "PATCH",
    json: { userId, actor: "player", actorId: userId, paymentMethod },
  });
}
...
export function createBookingPaymentRequest(
  bookingId: number,
  input: { requesterId: number; payerId: number; amount: number; purpose: "advance" | "booking"; note?: string },
): Promise<Record<string, unknown>> {
...
export function chooseBookingPaymentRequest(
  bookingId: number,
  requestId: number,
  userId: number,
  paymentMethod: "eSewa" | "Khalti",
