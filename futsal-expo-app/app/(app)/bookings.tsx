          ...
          <View style={styles.cardHeadRow}>
            ...
            <View style={styles.moneyCol}>
              ...
              <Text style={[styles.payMeta, { color: textFaint(muted) }]}>
                {b.paymentStatus === "paid" ? `Paid via ${method}` : `Payment method: ${method}`} • Payment status: {paymentStatusLabel(b.paymentStatus)}
              </Text>
            </View>
          </View>
          ...
          {b.teamName && teamShare ? (
            <View style={[styles.teamPaymentCard, { backgroundColor: isDark ? "rgba(14,165,233,0.12)" : "#F0F9FF", borderColor: isDark ? "rgba(56,189,248,0.25)" : "#BAE6FD" }]}>
              <View style={styles.teamPaymentHead}>
                ...
                <Text style={[styles.teamPaymentStatus, { color: teamShare.paymentStatus === "paid" ? "#047857" : muted }]}>{teamShare.paymentStatus === "paid" ? `Paid via ${teamShare.paymentMethod}` : teamShare.paymentMethod ? `Payment method: ${teamShare.paymentMethod}` : "Payment method not chosen"} • Payment status: {paymentStatusLabel(teamShare.paymentStatus)}</Text>
              </View>
