...
export default function VenueDetail() {
  ...
  async function book() {
    ...
    try {
      ...
      const needsGateway = serverTotal > 0 && isOnlineMethod(payMethod);
      if (needsGateway) {
        setPayRedirect(true);
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <SectionCard title="Your game plan" accent="emerald" highlighted>
            ...
            {total > 0 ? <PaymentBox methods={availableMethods} selected={payMethod} onSelect={setPayMethod} deposit={depositPreview} amount={depositPreview.required ? depositPreview.amount : total} receipt={receipt} onReceipt={setReceipt} /> : <Text style={[styles.freeMessage, { color: isDark ? colors.violet300 : colors.violet700 }]}>{promoDiscount > 0 ? "🎟️ Promo covers the whole bill — nothing to pay!" : "🎁 Fully covered — no payment needed!"}</Text>}
...
function PaymentBox({ methods, selected, onSelect, deposit, amount, receipt, onReceipt }: { methods: string[]; selected: string; onSelect: (value: string) => void; deposit: { required: boolean; percent: number; amount: number; reason: string }; amount: number; receipt: string; onReceipt: (value: string) => void }) {
  const { colors: c, isDark } = useTheme();
  return <View style={styles.paymentBox}><Text style={[styles.label, { color: c.textFaint }]}><Wallet size={13} color={c.textFaint} /> Pay your way</Text><View style={styles.paymentGrid}>{methods.map((method) => <Pressable key={method} onPress={() => onSelect(method)} style={[styles.paymentMethod, { backgroundColor: selected === method ? c.activeSoft : c.inset, borderColor: selected === method ? c.primary : c.border }]}><Wallet size={15} color={selected === method ? c.primary : c.textMuted} /><Text style={[styles.smallStrong, { color: selected === method ? c.activeText : c.textMuted }]}>{method}{method === "eSewa" ? " 💚" : method === "Khalti" ? " 💜" : ""}</Text>{selected === method ? <Check size={14} color={c.primary} /> : null}</Pressable>)}</View><Text style={[styles.tiny, { color: c.textFaint }]}>Online uses the server-backed test gateway. Cash is paid at the venue.</Text>{deposit.required ? <View style={[styles.depositBox, { backgroundColor: isDark ? "rgba(245,158,11,0.12)" : colors.amber300, borderColor: colors.amber400 }]}><Text style={[styles.smallStrong, { color: isDark ? colors.amber300 : "#78350F" }]}><ShieldAlert size={14} color={isDark ? colors.amber300 : "#78350F"} /> Fair-play deposit: {formatNPR(deposit.amount)} ({deposit.percent}%)</Text><Text style={[styles.tiny, { color: isDark ? colors.amber300 : "#92400E" }]}>{deposit.reason}</Text></View> : null}{isOnlineMethod(selected) ? <View style={[styles.gatewayInfo, { backgroundColor: isDark ? "rgba(14,165,233,0.10)" : "#F0F9FF", borderColor: isDark ? "rgba(14,165,233,0.3)" : "#BAE6FD" }]}><Text style={[styles.smallStrong, { color: isDark ? colors.sky300 : colors.sky700 }]}>{selected === "eSewa" ? "💚 eSewa TEST checkout" : "💜 Khalti TEST checkout"}</Text><Text style={[styles.tiny, { color: c.textMuted }]}>After Request, the app opens the server-backed test gateway and verifies the payment in the database.</Text><Text style={[styles.tinyStrong, { color: c.text }]}>Amount charged now: {formatNPR(amount)}</Text></View> : null}{isOnlineMethod(selected) ? <View style={[styles.receiptBox, { borderColor: c.border }]}><ReceiptUploader value={receipt} onChange={onReceipt} compact /></View> : null}</View>;
}
